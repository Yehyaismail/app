...
import {
  Send, Paperclip, Image, FileText, Download, Check, CheckCheck, Mic, Square,
  ArrowRight, Reply, Pencil, Trash2, X, Smile, Eraser, FileDown, UserPen, Video, Play
} from 'lucide-react';
...
const ReadReceipt = ({ status, isOwn }) => {
  ...
  if (status === 'delivered')
    return <CheckCheck className="w-4 h-4 text-slate-400 dark:text-slate-500 inline-block" />;
...
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const loadMedia = async () => {
    try {
      setLoadingFile(true);
      const res = await axios.get(`${API_URL}/api/files/${msg.file_url}`, {
      ...
      <div className="mb-2">
        {loadingFile ? (
          ...
          <img
            ...
            onClick={() => onOpenMedia(blobUrl, 'image', msg.file_name)}
  ...
  if (msg.message_type === 'video') {
    return (
      <div className="mb-2">
        {loadingFile ? (
          <div className="w-56 h-40 bg-slate-200 dark:bg-slate-700 animate-pulse rounded-lg flex items-center justify-center">
            <Video className="w-8 h-8 text-slate-400" />
          </div>
        ) : blobUrl ? (
          <div
            className="relative cursor-pointer group"
            onClick={() => onOpenMedia(blobUrl, 'video', msg.file_name)}
          >
            <video
              src={blobUrl}
          ...
          <div className="w-56 h-40 bg-slate-200 dark:bg-slate-700 rounded-lg flex items-center justify-center">
            <Video className="w-8 h-8 text-slate-400" />
          </div>
        )}
      </div>
    ...
    return (
      <div className="mb-1 flex items-center gap-2">
        <div className="flex-1">
          <VoicePlayer fileUrl={msg.file_url} duration={msg.voice_duration} />
        </div>
        <button
          onClick={handleDownload}
          className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg transition-colors flex-shrink-0"
          title="حفظ"
  ...
  if (msg.message_type === 'file') {
    return (
      <div
        className="mb-2 flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
        onClick={handleDownload}
...
export const ChatWindow = ({ selectedUser, currentUser, onNewMessage, onBack }) => {
  ...
  const handleScroll = () => {
    ...
    isUserScrolledUpRef.current = dist > 150;
  };
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (
        !e.target.closest('[data-testid="reaction-picker"]') &&
        !e.target.closest('[data-testid="react-msg-btn"]')
      ) {
      ...
      if (
        showEmojiPicker &&
        !e.target.closest('[data-testid="emoji-picker"]') &&
        !e.target.closest('[data-testid="emoji-btn"]')
      ) {
  ...
  // Typing Status
  // =========================
  const sendTypingStatus = async (t) => {
    if (!selectedUser) return;
    try {
      await axios.post(
        `${API_URL}/api/typing`,
        { receiver_id: selectedUser.id, is_typing: t },
        { withCredentials: true }
      );
    } catch (e) {}
  };
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (editingMsg) {
        await axios.put(
          `${API_URL}/api/messages/${editingMsg.id}`,
          { text: newMessage },
          { withCredentials: true }
        );
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const mt =
        up.data.category === 'image'
          ? 'image'
          : up.data.category === 'video'
          ? 'video'
          : 'file';
        ...
        `${API_URL}/api/messages`,
        {
          ...
          text:
            mt === 'image'
              ...
              : file.name,
          message_type: mt,
          file_url: up.data.storage_path,
          file_name: up.data.original_filename,
          file_type: up.data.content_type,
          reply_to: replyTo?.id || null
        },
        { withCredentials: true }
      );
  ...
  const handleDeleteMessage = async (msgId, mode = 'for_all') => {
    try {
      await axios.post(
        `${API_URL}/api/messages/${msgId}/delete`,
        { mode },
        { withCredentials: true }
      );
      await loadMessages();
  ...
  const handleExportChat = () => {
    if (!selectedUser) return;
    window.open(
      `${API_URL}/api/messages/${selectedUser.id}/export`,
      '_blank'
    );
  };
  ...
  const handleSetNickname = async () => {
    ...
    try {
      if (nicknameInput.trim()) {
        await axios.put(
          `${API_URL}/api/nicknames/${selectedUser.id}`,
  ...
  const handleReaction = async (msgId, emoji) => {
    try {
      await axios.post(
        `${API_URL}/api/messages/${msgId}/react`,
        { emoji },
        { withCredentials: true }
      );
      await loadMessages();
  ...
  // Open Media Viewer
  // =========================
  const openMedia = (src, type, fileName) =>
    setMediaView({ src, type, fileName });
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900">
        <img
          ...
          alt=""
          className="w-64 h-64 object-cover rounded-2xl opacity-40 dark:opacity-20 mb-6"
        />
        <p className="text-2xl text-slate-400 dark:text-slate-500 font-light">
          اختر محادثة للبدء
        </p>
      </div>
    );
  ...
  return (
    <div
      className="h-screen flex flex-col bg-slate-50 dark:bg-slate-900"
      style={chatStyle}
    >
      {/* =========================
          HEADER
      ...
      <div className="p-4 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {selectedUser.online && (
              <div className="absolute bottom-0 left-0 w-3 h-3 bg-emerald-500 border-2 border-white dark:border-slate-800 rounded-full"></div>
            )}
          </div>
            ...
            >
              <div
                className={`max-w-[70%] relative ${
                  ...
                  !settings.sentBubbleColor && isOwn
                    ? 'bg-emerald-100 dark:bg-emerald-900/40'
                    : ''
                } ${
                  !settings.receivedBubbleColor && !isOwn
                ...
                onContextMenu={(e) =>
                  !msg.deleted && openContextMenu(e, msg)
                }
              >
                {/* =========================
                    REPLY PREVIEW
                ...
                {repliedMsg && (
                  <div
                    className="mb-2 p-2 rounded-lg bg-slate-200/60 dark:bg-slate-700/60 border-r-2 border-emerald-500"
                ...
                {msg.deleted ? (
                  ...
                  <>
                    ...
                    {msg.message_type === 'text' && (
                      <p
                        className={`${fontSizeClass} leading-relaxed whitespace-pre-wrap`}
                      >
                        {msg.text}
                      </p>
                    )}
...
why do                 {/* =========================
                ...
                {!msg.deleted && (
                  <div
                    ...
                    } opacity-0 group-hover:opacity-100 transition-opacity flex gap-0.5`}
                  >
                    {/* Reaction Button */}
                    <div className="relative">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setReactionPicker(
                            reactionPicker === msg.id ? null : msg.id
                          );
                        }}
                        className="p-1.5 bg-white dark:bg-slate-700 rounded-md shadow-sm hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors"
                        data-testid="react-msg-btn"
                      >
                      ...
                      {reactionPicker === msg.id && (
                        <div
                          ...
                          } bg-white dark:bg-slate-800 rounded-full shadow-lg border border-slate-200 dark:border-slate-700 px-2 py-1 flex gap-1 z-30`}
                    ...
                    {isOwn && (
                      <button
                        onClick={() =>
                          setDeleteMenuMsg(
                            deleteMenuMsg === msg.id ? null : msg.id
                          )
                        }
                        className="p-1.5 bg-white dark:bg-slate-700 rounded-md shadow-sm hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors relative"
                        data-testid="delete-msg-btn"
                      >
                        <Trash2 className="w-3.5 h-3.5 text-red-400" />

                        {deleteMenuMsg === msg.id && (
                          <div
                            className="absolute top-8 left-0 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 py-1 z-30 w-36"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <button
                              onClick={() =>
                              ...
                              className="w-full px-3 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 text-right"
                              data-testid="delete-for-me-btn"
                            >
                              حذف لدي فقط
                            </button>
                            ...
                            <button
                              onClick={() =>
                                handleDeleteMessage(msg.id, 'for_all')
                              }
                              className="w-full px-3 py-2 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 text-right"
                              data-testid="delete-for-all-btn"
                            >
                              حذف للجميع
                            </button>
                          </div>
                    ...
                    {/* Delete for me (other user) */}
                    {!isOwn && (
                      <button
                        onClick={() =>
                          handleDeleteMessage(msg.id, 'for_me')
                        }
                        className="p-1.5 bg-white dark:bg-slate-700 rounded-md shadow-sm hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
                        data-testid="delete-for-me-other-btn"
                      >
                        <Trash2 className="w-3.5 h-3.5 text-red-400" />
                      </button>
                    )}
      ...
      {showClearConfirm && (
        <div
          ...
          data-testid="clear-chat-overlay"
        >
          <div
            className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-6 max-w-sm mx-4"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
              هل تريد حذف جميع رسائل هذه المحادثة نهائياً؟ لا يمكن التراجع عن هذا الإجراء.
            </p>
      ...
      {showNicknameDialog && (
        ...
        >
          <div
            className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-6 max-w-sm mx-4 w-full"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">
              تغيير اسم العرض
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              هذا الاسم يظهر لك فقط ولا يراه الطرف الآخر
            </p>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className="px-4 py-2 text-sm rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white transition-colors"
      ...
      {contextMenu && (
        ...
        >
          <div className="flex gap-1 px-3 py-2 border-b border-slate-100 dark:border-slate-700">
            {QUICK_REACTIONS.map((emoji) => (
              <button
                ...
                data-testid={`ctx-reaction-${emoji}`}
              >
                {emoji}
              </button>
            ))}
          ...
          <button
            ...
            className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700 text-sm text-slate-700 dark:text-slate-200 text-right"
            data-testid="ctx-reply-btn"
          >
          ...
          {contextMenu.msg.sender_id === currentUser?.id &&
            contextMenu.msg.message_type === 'text' && (
              <button
          ...
          {contextMenu.msg.sender_id === currentUser?.id && (
            <button
              onClick={() =>
                handleDeleteMessage(contextMenu.msg.id, 'for_all')
              }
              className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-red-50 dark:hover:bg-red-900/30 text-sm text-red-600 text-right"
          ...
          <button
            onClick={() =>
              handleDeleteMessage(contextMenu.msg.id, 'for_me')
            }
            className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700 text-sm text-slate-700 dark:text-slate-200 text-right"
            data-testid="ctx-delete-for-me-btn"
          >
            <Trash2 className="w-4 h-4" />
            <span>حذف لدي</span>
          </button>
      ...
      {(replyTo || editingMsg) && (
        <div
          className="px-4 py-3 bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex items-center gap-3"
          data-testid="reply-edit-bar"
        >
          ...
          <button
            ...
            data-testid="cancel-reply-edit-btn"
          >
        ...
        {isRecording ? (
          <div className="flex items-center gap-3" data-testid="voice-recording-bar">
            <button
              ...
              className="p-3 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-xl transition-colors"
            ...
            <div className="flex-1 flex items-center gap-3 px-4 py-3 bg-red-50 dark:bg-red-900/20 rounded-xl">
              ...
              <span
                className="text-red-500 dark:text-red-400 font-mono text-sm mr-auto"
                data-testid="recording-duration"
              >
                {formatRec(recordingDuration)}
              </span>
            </div>

            <Button
              onClick={stopRecordingAndSend}
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-6 rounded-xl"
          ...
          <form onSubmit={handleSend} className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              {showAttachMenu && (
                <div
                  className="absolute bottom-14 right-0 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-2 w-40 z-20"
  ...
  <span className="text-sm text-slate-700 dark:text-slate-200">
    ملف
            ...
            <input
              type="file"
              ref={fileInputRef}
              accept=".pdf,.doc,.docx,.txt,.xls,.xlsx,.zip,.rar,.mp3,.wav,.ogg,.aac"
              className="hidden"
              onChange={handleFileUpload}
            />
            ...
            <textarea
              ...
              placeholder={editingMsg ? 'عدّل الرسالة...' : 'اكتب رسالتك...'}
              className="flex-1 resize-none min-h-[44px] max-h-[120px] py-2.5 px-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-base leading-relaxed"
            ...
            {newMessage.trim() ? (
              <Button
                type="submit"
                disabled={loading || uploading}
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-6 rounded-xl transition-colors"
